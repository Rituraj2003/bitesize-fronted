...
export default function App() {
  ...
  const handleProcessReviewRating = async (id: string, performanceRating: 'easy' | 'hard') => {
    try {
          ...
          {view === 'dashboard' && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-xl">
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                Your production workspace layer is live. Your flashcards are safely synced across your live Render API engine and Neon's scalable cloud PostgreSQL database.
              </p>
            </div>
          )}
          ...
          {view === 'all-snippets' && (
            <div className="flex flex-col gap-6 w-full max-w-2xl">
              ...
              <div className="flex flex-col gap-4 w-full">
                {loading ? (
                  <p className="text-slate-500 italic text-sm animate-pulse">Running advanced text index search across cloud indices...</p>
                ) : snippets.length > 0 ? (
                  snippets.map(item => (
